...
function EmergencyContent() {
  ...
  const startCascade = useCallback((triggerGForce: number) => {
    ...
    CRASH_STEPS.forEach((step) => {
      const timer = setTimeout(() => {
        ...
        if (step.id === "nft") setNftUpdated(true)
        if (step.id === "escrow") setEscrowReleased(true)
  ...
  useEffect(() => {
    ...
    const lines = [
      `MAYDAY MAYDAY MAYDAY — AeroGuard autonomous agent for ${flight.callsign}`,
      ...
      `Telemetry anchored to IPFS — CID: ${crashCid}`,
      `XRPL Dynamic NFT updated — emergency status confirmed`,
      `Emergency escrow released — 100,000 RLUSD unlocked for SAR`,
      `Requesting immediate search and rescue response...`,
    ]
    const timers: ReturnType<typeof setTimeout>[] = []
    lines.forEach((line, i) => {
      timers.push(setTimeout(() => setTranscriptLines((prev) => [...prev, line]), 800 + i * 1800))
    })
    timers.push(setTimeout(() => setMaydayEnded(true), 800 + lines.length * 1800 + 1000))
    return () => timers.forEach(clearTimeout)
  }, [maydayActive, flight, gForce, crashCid])
  ...
  useEffect(() => {
    if (!maydayActive || maydayEnded) return
          ...
          {maydayActive && (
            <div className="animate-fade-slide-in">
              ...
              <div className="rounded-lg border border-red-500/25 bg-red-500/[0.03] overflow-hidden animate-pulse-red">
                ...
                <div className="flex items-center gap-3 px-3 py-2 border-b border-red-500/10 bg-red-500/[0.03]">
                  ...
                  <div className="ml-auto flex items-center gap-1.5">
                    ...
                    <span className="text-[8px] font-mono text-white/25">
                      {maydayEnded ? "ENDED" : "TRANSMITTING"}
                    </span>
                  </div>
                </div>
                ...
                {/* Transcript */}
                <div className="max-h-24 overflow-y-auto px-3 py-2 space-y-0.5 emergency-scroll">
                  {transcriptLines.map((line, i) => (
                    <p
                      key={i}
                      className="text-[10px] font-mono leading-relaxed animate-fade-slide-in"
                      style={{ color: i === transcriptLines.length - 1 ? "rgba(239,68,68,0.85)" : "rgba(255,255,255,0.35)" }}
                    >
                      {line}
                    </p>
                  ))}
